...
class PropertyEdit extends Component
{
    ...
    protected $listeners = ['deleteImgProperty'];
    ...
    public function propertyEdit()
    {
        ...
        $editar = Property::find($this->property_idm);
        // editar datos
        $editar->update($datos);
        ...
        if($this->img_property){
            ...
            foreach($this->img_property as $img_files){
                ...
                \App\Models\Base\Image::create([
                    'property_id' => $this->property_idm,
                    'name' => $file_name,
                    'type' => 'properties',
                ]);
            }
    ...
    // eliminar imagenes de propiedad
    public function deleteImgProperty(\App\Models\Base\Image $dato){
        if(File::exists('img/img_properties/'.$dato->name)){
            File::delete('img/img_properties/'.$dato->name);
        }
        $dato->delete();
    }    
    ...
    public function render()
    {
        ...
        $img_properties_files = \App\Models\Base\Image::where('property_id', '=' , $this->property_idm)->where('type', '=' , 'properties')->get();
        $viewShow = false;
        return view('livewire.admin.properties.property-edit',[
